...
const OBOJIMA_POTION_INVENTORY_KEY = "obojimaPotionInventory";
...
const Obojima = (() => {
    ...
    function loadPotionInventory() {
        try {
            const stored = JSON.parse(localStorage.getItem(OBOJIMA_POTION_INVENTORY_KEY) || "{}");
            ...
            console.warn("Unable to load potion inventory.", error);
            return {};
        }
    }

    function savePotionInventory(potions) {
        const normalized = {};
        Object.entries(potions || {}).forEach(([key, value]) => {
            const quantity = Math.max(0, Math.floor(Number(value) || 0));
            if (quantity > 0) normalized[key] = quantity;
        });
        localStorage.setItem(OBOJIMA_POTION_INVENTORY_KEY, JSON.stringify(normalized));
        return normalized;
    }

    function setPotionQuantity(key, quantity) {
        const potions = loadPotionInventory();
        const nextQuantity = Math.max(0, Math.floor(Number(quantity) || 0));
        if (nextQuantity > 0) potions[key] = nextQuantity;
        else delete potions[key];
        return savePotionInventory(potions);
    }

    function incrementPotionQuantity(key, delta) {
        const potions = loadPotionInventory();
        return setPotionQuantity(key, (potions[key] || 0) + delta);
    }
    ...
    function clearInventoryProfile() {
        ...
        localStorage.removeItem(OBOJIMA_POTION_INVENTORY_KEY);
    ...
    async function exportInventory(items) {
        ...
        if (inventory.length === 0 && Object.keys(loadPotionInventory()).length === 0 && !currentProfile.playerName && !currentProfile.characterName) {
    ...
    return {
            ...
            quantities,
            potions: parsed && !Array.isArray(parsed) && parsed.potions && typeof parsed.potions === "object"
                ? parsed.potions
                : {}
        };
    }
        ...
        input.addEventListener("change", () => {
            ...
            reader.onload = () => {
                try {
                    ...
                    savePotionInventory(imported.potions || {});
